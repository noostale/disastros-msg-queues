...
void internal_MessageQueue_read() {
  ...
  **/
  ...
  printf("METTO IN ATTESA UN PROCESSO CHE VOLEVA LEGGERE UNA MQ VUOTA\n");
  ...
  printf("Ho appena letto dalla mq di id:%d, il messaggio '%s'\n",id, first_message->message);


  while(mq->waiting_to_write.size > 0){

    printf("FACCIO RIPARTIRE UN WRITER CHE VOLEVA SCRIVERE IN UNA MQ PIENA\n");
